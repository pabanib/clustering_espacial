...
DIR_DATOS_PROC = os.path.join(DIR_DATOS,"procesados/covid")
#dir_datos = DIR_DATOS+'\\originales'

def __genera_shp():
    covid = pd.read_csv(DIR_DATOS_ORIG+'/Covid19Casos.csv')
    ...
    import pickle 
    with open(DIR_DATOS_ORIG+"/covid_geo.pickle", "wb") as f:
    ...
    # completo con valor cero los departamentos que en alguna fecha aún on tengan datos
    ...
    covid.to_file(DIR_DATOS_PROC+'/covid_periodos.shp', index = True)

def df_covid(dir_datos = DIR_DATOS_PROC):
