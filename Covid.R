...
wss_redcap <- elegir_grupos(covid, metodo =redcap, grupos =grupos)
...
write.csv(rdos, "Resultados/covid_R.csv")

sk2 <- skater(10, w_knn, covid, min_bound = 30, bound_variable = as.data.frame(rep(1,525)))
sk2$`Total within-cluster sum of squares`
sk2$`The ratio of between to total sum of squares`
sk2$`Total sum of squares`

plot(st_geometry(covid), col = sk2$Clusters)
